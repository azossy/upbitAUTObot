...
String getApiErrorMessage(dynamic e, {String fallback = '오류가 발생했습니다.'}) {
  if (e is DioException) {
    if (e.type == DioExceptionType.connectionError ||
      ...
      return '서버에 연결할 수 없습니다. 설정에서 API 서버 주소와 백엔드 실행 여부를 확인하세요.';
    }
    final data = e.response?.data;
    String? rawMessage;
    if (data is Map && data['detail'] != null) {
      final d = data['detail'];
      rawMessage = d is String ? d : d.toString();
    }
    if (e.response?.statusCode != null) {
      ...
      if (e.response!.statusCode == 404) {
        return '서버 주소를 확인해 주세요. 앱 설정에서 API 서버 주소가 맞는지, 서버가 켜져 있는지 확인하세요.';
      }
    }
    final msg = rawMessage ?? e.message?.toString() ?? fallback;
    // 서버/영문 오류 문구를 사용자 친화적 한글 안내로 치환
    if (msg.toLowerCase().contains('not found') || msg == 'Not Found') {
      return '서버에 연결할 수 없습니다. 설정에서 API 서버 주소를 확인해 주세요.';
    }
    if (msg.toLowerCase().contains('connection') || msg.toLowerCase().contains('failed host')) {
      return '서버에 연결할 수 없습니다. 네트워크와 API 서버 주소를 확인해 주세요.';
    }
    if (e.response?.statusCode == 503) {
      if (msg.contains('구글') || msg.contains('google')) return '구글 로그인 기능이 서버에 설정되지 않았습니다. 서버 관리자에게 문의해 주세요.';
      if (msg.contains('카카오') || msg.contains('kakao')) return '카카오 로그인 기능이 서버에 설정되지 않았습니다. 서버 관리자에게 문의해 주세요.';
      if (msg.contains('이메일') || msg.contains('메일')) return '이메일 발송 기능이 설정되지 않았습니다. 인증 메일이 필요하면 서버 관리자에게 문의해 주세요.';
      return '일시적으로 사용할 수 없는 기능입니다. 잠시 후 다시 시도해 주세요.';
    }
    if (e.response?.statusCode == 400 && msg.isNotEmpty) return msg;
    return msg;
  }
  final s = e.toString();
  if (s.toLowerCase().contains('not found')) return '서버에 연결할 수 없습니다. 설정에서 API 서버 주소를 확인해 주세요.';
  return s.isNotEmpty ? s : fallback;
}

class ApiService {
  /// 기본 서버: 우리 Jetson 서버 (Tailscale). 서버가 바뀌면 앱 설정에서 새 주소 입력 후 저장.
  static const String defaultBaseUrl = 'http://100.80.178.45:8000';
  ...
  /// 구글 로그인. 응답: need_register true면 email/name, false면 access_token/user
  ...
  /// 카카오 로그인. 응답: need_register true면 email/name, false면 access_token/user
  ...
  /// 구글 OAuth 회원가입 완료 (미가입 사용자가 닉네임 입력 후)
  Future<Map<String, dynamic>> completeGoogleRegister(String idToken, String nickname) async {
    final res = await _dio.post('/api/v1/auth/complete-google-register', data: {
      'id_token': idToken,
  ...
  /// 카카오 OAuth 회원가입 완료 (미가입 사용자가 닉네임 입력 후)
  Future<Map<String, dynamic>> completeKakaoRegister(String accessToken, String nickname) async {
    final res = await _dio.post('/api/v1/auth/complete-kakao-register', data: {
  ...
  Future<void> sendVerificationEmail(String email) async {
    await _dio.post('/api/v1/auth/send-verification-email', data: {'email': email});
  }

  /// 회원가입 2단계: 인증 번호 확인 후 가입 완료
  Future<Map<String, dynamic>> verifyAndRegister({
    ...
    final res = await _dio.post('/api/v1/auth/verify-and-register', data: {
      'email': email,
      'password': password,
      'nickname': nickname,
      'code': code,
