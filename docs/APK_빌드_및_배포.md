...
2. 빌드가 끝나면 `upbit_trading_app\build\app\outputs\flutter-apk\baejjangi-1-4-8.apk`(예: 버전 1.4.8) 형식으로 생성됩니다. (버전마다 파일명이 `baejjangi-X-Y-Z.apk` 로 붙습니다.)
3. **GitHub CLI(gh)** 가 설치되어 있다면, 프로젝트 루트에서:
   ...
   ```
   (이미 v1.4.8 Release가 있어야 합니다. 없으면 먼저 `gh release create v1.4.8 --title "배짱이 v1.4.8"` 로 생성.)
...
2. `upbit_trading_app` 폴더에서 PowerShell을 열고:
   ```powershell
   .\upload_apk_to_github.ps1 v1.4.8
   ```
...
gh release upload v1.4.8 build/app/outputs/flutter-apk/baejjangi-1-4-8.apk --repo azossy/upbitAUTObot --clobber
```
...
2. **Build Release APK** 워크플로가 자동으로 실행되어 APK를 빌드한 뒤, **반드시 `baejjangi-버전.apk`**(예: baejjangi-1-4-5.apk)로 이름을 바꿔 해당 Release에 업로드합니다. (Flutter 기본 출력 `app-release.apk` → 워크플로에서 pubspec version 기준으로 변경.)
...
**※ 다음 빌드 시에도 적용**: 워크플로(`.github/workflows/build-apk.yml`)에서 APK 출력 파일명을 **baejjangi-버전.apk** 로 고정해 두었습니다. 수정 시 이 규칙을 유지하세요.
...
- **버전**: `upbit_trading_app/pubspec.yaml` 의 `version` 과 `lib/constants/app_version.dart` 의 `kAppVersion` 을 배포 버전에 맞게 올려주세요. (예: 1.4.8)
