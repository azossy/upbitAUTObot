...
$cmd = "cd ~/upbitAUTObot && git pull && sudo systemctl restart upbit-backend && echo DONE && sudo systemctl status upbit-backend --no-pager"

Write-Host "Jetson($remote)에 접속해 git pull 및 upbit-backend 재시작을 실행합니다." -ForegroundColor Cyan
